...
function applyFilters(){
  ...
  if(sf!=='all') items=items.filter(i=>i.best_store===sf || (i.sources_summary||[]).some(src=>src.store_name===sf));
...
function renderTable(items){ const tbody=document.querySelector('#prices-table tbody'); tbody.innerHTML=''; items.forEach(i=>{ const srcHtml=(i.sources_summary||[]).map(s=>`<li><strong>${s.store_name}</strong>: ${s.status}${s.price!=null?` · ${PLN.format(s.price)}`:''}${s.availability?` · ${s.availability}`:''}${s.url?` · <a href="${s.url}" target="_blank" rel="noopener">link</a>`:''}</li>`).join(''); const tr=document.createElement('tr'); tr.innerHTML=`<td><details><summary>${i.model_name}</summary><div class="details">${statusBadge(i)} · źródeł OK: ${i.sources_ok ?? 0}/${i.sources_checked ?? 0}<ul>${srcHtml}</ul></div></details></td><td>${i.ram_gb ?? '-'} / ${i.storage_gb ?? '-'} GB</td><td>${fmtPrice(i.best_price)}</td><td>${i.best_store ?? '-'}</td><td>${i.price_change!=null?PLN.format(i.price_change):'-'}</td><td>${fmtPrice(i.historical_min_price)}</td><td>${fmtPrice(i.historical_max_price)}</td><td>${i.availability ?? '-'}</td><td>${fmtDate(i.last_success_at)}</td><td>${i.best_url?`<a href="${i.best_url}" target="_blank" rel="noopener">Oferta</a>`:'-'}</td>`; tbody.appendChild(tr); }); }
function renderFilters(){ const stores=[...new Set(state.items.flatMap(i=>(i.sources_summary||[]).map(s=>s.store_name)).filter(Boolean))]; const sel=document.getElementById('store-filter'); sel.innerHTML='<option value="all">Wszystkie sklepy</option>'+stores.map(s=>`<option>${s}</option>`).join(''); ['search','store-filter','status-filter','sort-by'].forEach(id=>document.getElementById(id).addEventListener('input',applyFilters)); }
